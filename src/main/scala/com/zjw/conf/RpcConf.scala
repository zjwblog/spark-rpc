...
import com.zjw.internal.config.{ConfigEntry, ConfigProvider, ConfigReader, OptionalConfigEntry, SparkConfigProvider, _}
import com.zjw.serializer.KryoSerializer
 ...
 */
class RpcConf(loadDefaults: Boolean) extends Cloneable with Logging with Serializable {
  ...
  private[zjw] def loadFromSystemProperties(silent: Boolean): RpcConf = {
  ...
  private[zjw] def set(key: String, value: String, silent: Boolean): RpcConf = {
    if (key == null) {
  ...
  private[zjw] def set[T](entry: ConfigEntry[T], value: T): RpcConf = {
    set(entry.key, entry.stringConverter(value))
    this
  }

  private[zjw] def set[T](entry: OptionalConfigEntry[T], value: T): RpcConf = {
   ...
   */
  def setMaster(master: String): RpcConf = {
    set("spark.master", master)
  }

  /** Set a name for your application. Shown in the Spark web UI. */
  def setAppName(name: String): RpcConf = {
    set("spark.app.name", name)
  }

  /** Set JAR files to distribute to the cluster. */
  def setJars(jars: Seq[String]): RpcConf = {
    for (jar <- jars if (jar == null)) logWarning("null jar passed to SparkContext constructor")
    set("spark.jars", jars.filter(_ != null).mkString(","))
  }

  /** Set JAR files to distribute to the cluster. (Java-friendly version.) */
  def setJars(jars: Array[String]): RpcConf = {
    setJars(jars.toSeq)
  }
   ...
   */
  def setExecutorEnv(variable: String, value: String): RpcConf = {
   ...
   * (for example spark.executorEnv.PATH) but this method makes them easier to set.
   */
  def setExecutorEnv(variables: Seq[(String, String)]): RpcConf = {
    for ((k, v) <- variables) {
      setExecutorEnv(k, v)
    }
    this
   ...
   */
  def setExecutorEnv(variables: Array[(String, String)]): RpcConf = {
   ...
   */
  def setSparkHome(home: String): RpcConf = {
    set("spark.home", home)
  }

  /** Set multiple parameters together */
  def setAll(settings: Traversable[(String, String)]): RpcConf = {
  ...
  private[zjw] def setIfMissing[T](entry: ConfigEntry[T], value: T): RpcConf = {
  ...
  private[zjw] def setIfMissing[T](entry: OptionalConfigEntry[T], value: T): RpcConf = {
   ...
   */
  def registerKryoClasses(classes: Array[Class[_]]): RpcConf = {
   ...
   */
  def registerAvroSchemas(schemas: Schema*): RpcConf = {
    for (schema <- schemas) {
  ...
  private[zjw] def remove(entry: ConfigEntry[_]): RpcConf = {
    remove(entry.key)
  }
   ...
   */
  private[zjw] def get[T](entry: ConfigEntry[T]): T = {
  ...
  private[zjw] def getWithSubstitution(key: String): Option[String] = {
  ...
  /** Get all executor environment variables set on this RpcConf */
  ...
  private[zjw] def contains(entry: ConfigEntry[_]): Boolean = contains(entry.key)

  /** Copy this object */
  override def clone: RpcConf = {
    val cloned = new RpcConf(false)
   ...
   * in unit tests.
   */
  private[zjw] def getenv(name: String): String = System.getenv(name)

  /**
   * Checks for illegal or deprecated config settings. Throws an exception for the former. Not
   * idempotent - may mutate this conf object to convert deprecated settings to supported ones.
   */
  private[zjw] def validateSettings() {
    ...
    getOption(executorOptsKey).foreach { javaOpts =>
      if (javaOpts.contains("-Dspark")) {
        val msg = s"$executorOptsKey is not allowed to set Spark options (was '$javaOpts'). " +
          "Set them directly on a RpcConf or in a properties file when using ./bin/spark-submit."
    ...
    if (contains("spark.submit.deployMode")) {
      get("spark.submit.deployMode") match {
        case "cluster" | "client" =>
        case e => throw new RPCException("spark.submit.deployMode can only be \"cluster\" or " +
          "\"client\".")
      }
    }
...
private[zjw] object RpcConf extends Logging {
