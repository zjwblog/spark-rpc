...
import com.zjw.serializer.{DeserializationStream, SerializationStream, SerializerInstance}
 ...
 */
private[zjw] case class CallSite(shortForm: String, longForm: String)

private[zjw] object CallSite {
 ...
 */
private[zjw] object Utils extends Logging {
...
//  }
   ...
   *
   * Throws RPCException if the target file already exists and has different contents than
   * the requested file.
   */
//  def fetchFile(
//                 url: String,
...
//  }
   ...
   */
  def getLocalDir(conf: RpcConf): String = {
  ...
  private[zjw] def isRunningInYarnContainer(conf: RpcConf): Boolean = {
   ...
   * set of directories.
   */
  private[zjw] def getOrCreateLocalRootDirs(conf: RpcConf): Array[String] = {
   ...
   */
  def getConfiguredLocalDirs(conf: RpcConf): Array[String] = {
  ...
  private def getOrCreateLocalRootDirsImpl(conf: RpcConf): Array[String] = {
    val configuredLocalDirs = getConfiguredLocalDirs(conf)
  ...
  private def getYarnLocalDirs(conf: RpcConf): String = {
  ...
  private[zjw] def clearLocalRootDirs(): Unit = {
    localRootDirs = null
  }
  ...
  def executeAndGetOutput(
    ...
    if (exitCode != 0) {
      ...
      throw new RPCException(s"Process $command exited with code $exitCode")
    }
    output.toString
  }
...
//  }
  ...
  private def getCompressedLogFileLengthCache(
                                               sparkConf: RpcConf): LoadingCache[String, java.lang.Long] = this.synchronized {
   ...
   */
  def getFileLength(file: File, workConf: RpcConf): Long = {
...
//  }
   ...
   */
  def loadDefaultSparkProperties(conf: RpcConf, filePath: String = null): String = {
  ...
  def getPropertiesFromFile(filename: String): Map[String, String] = {
    ...
    try {
      ...
      case e: IOException =>
        throw new RPCException(s"Failed when loading Spark properties from $filename", e)
    } finally {
      inReader.close()
    }
  }
...
//  }
   ...
   */
  def sparkJavaOpts(conf: RpcConf, filterKey: (String => Boolean) = _ => true): Seq[String] = {
    conf.getAll
...
//  }
  ...
  def getMaxResultSize(conf: RpcConf): Long = {
  ...
  /**
   * Return the value of a config either through the RpcConf or the Hadoop configuration
   * if this is Yarn mode. In the latter case, this defaults to the value set through RpcConf
   * if the key is not set in the Hadoop configuration.
   */
//  def getSparkOrYarnConfig(conf: RpcConf, key: String, default: String): String = {
...
//  def getCurrentUserGroups(sparkConf: RpcConf, username: String): Set[String] = {
...
//  }
   ...
   */
  def isLocalMaster(conf: RpcConf): Boolean = {
   ...
   */
  def isDynamicAllocationEnabled(conf: RpcConf): Boolean = {
   ...
   */
//  def getDynamicAllocationInitialExecutors(conf: RpcConf): Int = {
...
//  }
   ...
   */
  def getUserJars(conf: RpcConf): Seq[String] = {
   ...
   */
  def getLocalUserJarsForShell(conf: RpcConf): Seq[String] = {
    val localJars = conf.getOption("spark.repl.local.jars")
    localJars.map(_.split(",")).map(_.filter(_.nonEmpty)).toSeq.flatten
  }

  private[zjw] val REDACTION_REPLACEMENT_TEXT = "*********(redacted)"

  /**
   * Redact the sensitive values in the given map. If a map key matches the redaction pattern then
   * its value is replaced with a dummy text.
   */
//  def redact(conf: RpcConf, kvs: Seq[(String, String)]): Seq[(String, String)] = {
...
//  }
   ...
   */
  def loadExtensions[T](extClass: Class[T], classes: Seq[String], conf: RpcConf): Seq[T] = {
    classes.flatMap { name =>
      try {
        ...
        val ext = Try(klass.getConstructor(classOf[RpcConf])) match {
          case Success(ctor) =>
        ...
        case _: NoSuchMethodException =>
          throw new RPCException(
            s"$name did not have a zero-argument constructor or a" +
              " single-argument constructor that accepts RpcConf. Note: if the class is" +
...
//  def createSecret(conf: RpcConf): String = {
...
//private[zjw] class CallerContext(
//                                    from: String,
 ...
 */
private[zjw] class RedirectThread(
                                     in: InputStream,
                                     out: OutputStream,
                                     name: String,
 ...
 * the toString method.
 */
private[zjw] class CircularBuffer(sizeInBytes: Int = 10240) extends java.io.OutputStream {
