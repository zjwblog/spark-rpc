...
import com.zjw.util.{ByteBufferInputStream, ByteUnit, Utils}
 ...
 */
class KryoSerializer(conf: RpcConf)
  extends Serializer
    with Logging
    with Serializable {
  ...
  def newKryo(): Kryo = {
    ...
    try {
      ...
      case e: Exception =>
        throw new RPCException(s"Failed to register classes with Kryo", e)
    } finally {
  ...
  private[zjw] override lazy val supportsRelocationOfSerializedObjects: Boolean = {
...
private[zjw] class KryoSerializationStream(
...
private[zjw] class KryoDeserializationStream(
                                 serInstance: KryoSerializerInstance,
...
private[zjw] class KryoSerializerInstance(ks: KryoSerializer, useUnsafe: Boolean)
  ...
  override def serialize[T: ClassTag](t: T): ByteBuffer = {
    ...
    try {
      ...
      case e: KryoException if e.getMessage.startsWith("Buffer overflow") =>
        throw new RPCException(s"Kryo serialization failed: ${e.getMessage}. To avoid this, " +
 ...
 */
private[zjw] class KryoInputObjectInputBridge(
 ...
 */
private[zjw] class KryoOutputObjectOutputBridge(
