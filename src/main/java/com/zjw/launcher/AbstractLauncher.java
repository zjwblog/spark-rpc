...
import static com.zjw.launcher.CommandBuilderUtils.checkNotNull;
...
public abstract class AbstractLauncher<T extends AbstractLauncher> {
  ...
  /**
   * Adds an argument with a value to the Spark invocation. If the argument name corresponds to a
   * known argument, the code validates that the argument actually expects a value, and throws an
   * exception otherwise.
   * <p>
   * It is safe to add arguments modified by other methods in this class (such as {@link
   * #setMaster(String)} - the last invocation will be the one to take effect.
   * <p>
   * Use this method with caution. It is possible to create an invalid Spark command by passing
   * unknown arguments to this method, since those are allowed for forward compatibility.
   *
   ...
   *
   * <p>
   * This method returns a handle that provides information about the running application and can be
   * used to do basic interaction with it.
   * <p>
   * The returned handle assumes that the application will instantiate a single SparkContext during
   * its lifetime. Once that context reports a final state (one that indicates the SparkContext has
   * stopped), the handle will not perform new state transitions, so anything that happens after
   * that cannot be monitored. If the underlying application is launched as a child process, {@link
   * SparkAppHandle#kill()} can still be used to kill the child process.
   *
